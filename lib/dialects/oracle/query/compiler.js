...
class QueryCompiler_Oracle extends QueryCompiler {
  ...
  // oracle doesn't use recursive keyword, but expects column definition for recursive CTEs
  with() {
  ...
  withWrapped(statement) {
    ...
    if (statement.recursive && statement.columns instanceof Array) {
      const columns = statement.columns
        .map((column) => this.formatter.columnize(column))
        .join(', ');
      return (
        (val &&
          this.formatter.columnize(statement.alias) +
            ' (' +
            columns +
            ') as (' +
            val +
