...
const validateWithArgs = function(alias, statement, method, columns) {
  ...
  if (
    ...
    throw new Error(`${method}() columns have to be strings`);
  }
  if (
...
assign(Builder.prototype, {
  ...
  withRecursive(alias, statement, columns) {
    validateWithArgs(alias, statement, 'withRecursive', columns);
    return this.withRecursiveWrapped(alias, statement, columns);
  },

  // Helper for compiling any advanced `withRecursive` queries.
  withRecursiveWrapped(alias, query, columns) {
    this.withWrapped(alias, query);
    this._statements[this._statements.length - 1].recursive = true;
    this._statements[this._statements.length - 1].columns = columns;
    return this;
  },
